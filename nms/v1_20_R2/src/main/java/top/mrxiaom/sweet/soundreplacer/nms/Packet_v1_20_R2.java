...
public class Packet_v1_20_R2 implements PacketAPI {
    @Override
    ...
    public @Nullable Object overrideSound(PacketContainer packet, NamespacedKey sound) {
        ...
        if (handle instanceof PacketPlayOutNamedSoundEffect p) {
            return new PacketPlayOutNamedSoundEffect(holder, p.d(), p.e(), p.f(), p.g(), p.h(), p.i(), p.j());
        }
        if (handle instanceof PacketPlayOutEntitySound p) {
            return new PacketPlayOutEntitySound(holder, p.d(), new DummyEntity(p.e()), p.f(), p.g(), p.h());
        }
        return null;
    }
